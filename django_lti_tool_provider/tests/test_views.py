...
class LtiRequestsTestBase(TestCase):
    ...
    def send_lti_request(self, payload, client=None):
        ...
        return client.post(
            "/lti/", payload, content_type="application/x-www-form-urlencoded"
        )
...
class AnonymousLtiRequestTests(LtiRequestsTestBase):
    def setUp(self):
        ...
        self.hook_manager.anonymous_redirect_to = Mock(
            return_value=self.DEFAULT_REDIRECT
        )
...
class AuthenticatedLtiRequestTests(LtiRequestsTestBase):
    def _authentication_hook(
        self, request, user_id=None, username=None, email=None, **kwargs
    ):
        user = User.objects.create_user(
            username or user_id, password="1234", email=email
        )
        user.save()
        authenticated_user = authenticate(
            request, username=user.username, password="1234"
        )
    ...
    def setUp(self):
        ...
        self.hook_manager.authenticated_redirect_to = Mock(
            return_value=self.DEFAULT_REDIRECT
        )
        self.hook_manager.authentication_hook = self._authentication_hook

    def _verify_lti_updated_signal_is_sent(
        self, patched_send_lti_received, expected_user
    ):
        expected_lti_data = LtiUserData.objects.get(user=expected_user)
        patched_send_lti_received.assert_called_once_with(
            LTIView, user=expected_user, lti_data=expected_lti_data
        )
    ...
    def test_no_session_correct_payload_processes_lti_request(
        self, patched_send_lti_received
    ):
        # Precondition check
    ...
    def test_force_login_change(self, patched_send_lti_received):
        ...
        request.user = None
        authenticated_user = authenticate(
            request, username=new_user.username, password="1234"
        )
...
class AuthenticationManagerIntegrationTests(LtiRequestsTestBase):
    ...
    def test_authentication_hook_passes_optional_lti_data(self):
        payload = self.get_correct_lti_payload()
        self.hook_manager.optional_lti_parameters.return_value = {
    ...
    @ddt.data(*TEST_URLS)
    def test_anonymous_lti_is_processed_if_hook_does_not_authenticate_user(
        self, expected_url
    ):
